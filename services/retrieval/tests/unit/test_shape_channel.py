...
from ikip_contracts import AclPolicy, Authority, RetrievalChannel
...
def test_d2_descriptor_translation_invariant() -> None:
    ...
    sim = sum(x*y for x, y in zip(da, db))
...
def test_restricted_shape_candidate_never_ranked() -> None:
    ...
    acls.upsert(AclPolicy(
        document_id="doc-allowed", owner="eng", sites=["site-a"],
        roles_allowed=["eng"], source_of_truth="test",
        synced_at=now, max_staleness_seconds=3600,
    ))
    acls.upsert(AclPolicy(
        document_id="doc-restricted", owner="eng", sites=["site-b"],
        roles_allowed=["eng"], source_of_truth="test",
        synced_at=now, max_staleness_seconds=3600,
    ))
    ...
    ctx = AuthorizationContext(
        subject_id="u1", roles=frozenset({"eng"}),
        sites=frozenset({"site-a"}), identity_verified=True,
    )
